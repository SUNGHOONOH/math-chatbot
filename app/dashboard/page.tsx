    ...
    <main className="min-h-screen bg-slate-50 text-slate-900 p-6 md:p-10">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <header className="flex justify-between items-center">
          ...
          <Link
            href="/"
            className="px-4 py-2 bg-white hover:bg-slate-100 text-slate-700 text-sm font-medium rounded-xl transition-all border border-slate-200 flex items-center gap-2 shadow-sm"
          >
            🏠 홈으로
          </Link>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <section className="bg-white border border-slate-200 shadow-sm rounded-2xl p-6 space-y-4">
            ...
            {masteredRanking.length === 0 ? (
              <p className="text-slate-400 text-sm">아직 데이터가 없습니다. 문제를 풀어보세요!</p>
            ) : (
              <ul className="space-y-3">
                {masteredRanking.map((item, i) => (
                  <li key={item.concept} className="flex items-center justify-between bg-slate-50 border border-slate-100 rounded-lg px-4 py-3">
                    <div className="flex items-center gap-3">
                      <span className="text-emerald-500 font-bold text-lg">{i + 1}</span>
                      <span className="text-sm font-mono text-slate-700">{item.concept}</span>
                    </div>
                    <span className="text-xs text-slate-500">{item.count}회 마스터</span>
                  </li>
                ))}
              </ul>
            )}
          ...
          <section className="bg-white border border-slate-200 shadow-sm rounded-2xl p-6 space-y-4">
            ...
            {weakRanking.length === 0 ? (
              <p className="text-slate-400 text-sm">취약 개념이 없습니다. 훌륭합니다! 🎉</p>
            ) : (
              <ul className="space-y-3">
                {weakRanking.map((item, i) => (
                  <li key={item.concept} className="bg-slate-50 border border-slate-100 rounded-lg px-4 py-3 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className="text-rose-500 font-bold text-lg">{i + 1}</span>
                        <span className="text-sm font-mono text-slate-700">{item.concept}</span>
                      </div>
                      <span className="text-xs text-slate-500">{item.count}회 미해결</span>
                    </div>
                    <Link
                      href={`/practice?concept=${encodeURIComponent(item.concept)}`}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-zinc-900 hover:bg-zinc-800 text-white text-xs font-medium rounded-lg transition-all duration-200 shadow-sm"
                    >
                      🎯 이 개념만 집중 훈련하기
                    </Link>
                  </li>
